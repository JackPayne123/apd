...
def train(
    data_dir: str = "./data",
    input_size: int = 28 * 28,  # KMNIST images are 28x28
    hidden_size: int = 512,
    num_classes: int = 10,  # 10 classes in KMNIST (Kuzushiji characters)
    num_epochs: int = 3,
    batch_size: int = 128,
    ...
    full_dataset = datasets.KMNIST(root=data_dir, train=True, download=True, transform=transform)
    ...
    model = KMNISTModel(input_size, hidden_size, num_classes).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for images, labels in loop:
            images = images.view(-1, input_size).to(device)  # Flatten the 28x28 images
    ...
    torch.save(model.state_dict(), "models/kmnist_model.pth")
